...
void SxEESGProj::cardBSplineAll (double xx, int p, SxVecRef<double> &res)
{
...
SxVector<SxComplex16> SxEESGProj::getDp (int p, SxFFT1d &fft)
{
   SX_CLOCK (DpSetup);
...
SxEESGProj::projectGrad (const SxVecRef<SxComplex16> &refPhi,
                         ...
                         const SxAtomicStructure     &centers) const
{
   ...
   SxVector<SxComplex16> pG(*gPtr), pS;
   {
      // --- set up pG(ig) = psi(ig).conj () * phi(ig) * prod_dir d_p(dir,ig)
      // Equation 15b of ref. 1
      SX_CLOCK (PhiPsiDp);
      SxVector<SxComplex16>::Iterator phiIt = refPhi.begin (),
                                      psiIt = psi.begin (),
                                      pGIt  = pG.begin ();
   ...
   SxVector<SxComplex16> res(4, centers.getNAtoms ());
   ...
   for (int ia = 0; ia < centers.getNAtoms (); ++ia)  {
      ...
      // --- perform sum of Eq. 14
      {
         ...
         SxVector<SxComplex16>::Iterator pIt;
         ...
         res(0,ia) = sqrt(S.cell.volume) * sum;
         {
            ...
            const SxMesh3D &m = S.fft3d.mesh;
            SxVector3<SxComplex16> fVec(fx * double(m(0)),
                                        fy * double(m(1)),
                                        fz * double(m(2)));
            SxVector3<SxComplex16> fAbs
               = (S.cell.inverse ().transpose () ^ fVec);
            // prefactor
...
SxVector<SxComplex16>
SxEESGProj::project (const SxVecRef<SxComplex16> &refPhi,
                     const SxVecRef<SxComplex16> &psi,
                     const SxAtomicStructure     &centers) const
{
   SX_CLOCK (Projection);
   ...
   SxVector<SxComplex16> res(centers.getNAtoms ());
   SxVector<double> mpx(splineOrder), mpy(splineOrder), mpz(splineOrder),
                    work(splineOrder);
   for (int ia = 0; ia < centers.getNAtoms (); ++ia)  {
      Coord pos = S.cell.carToRel(centers.getAtom(ia)) * S.fft3d.mesh;
...
SxVector<SxComplex16>
SxEESGProj::gradient (const SxVecRef<SxComplex16>   &proj,
                      const SxVecRef<SxComplex16> &refPhi,
                      const SxAtomicStructure     &centers) const
{
   SX_CLOCK (Gradient);
   ...
   SxVector<SxComplex16> pS(0., S), res;
   SxVector<double> mpx(splineOrder), mpy(splineOrder), mpz(splineOrder),
                    work(splineOrder);
...
void SxEESGProj::updateG (const SxGBasis *gPtr) const
{
   ...
   SxVector3<int> Grel;
   SxVector<PrecFFTIdx>::Iterator n123It = gPtr->n123(sBasisIdx).begin ();
